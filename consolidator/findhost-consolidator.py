...
NISFOLDER = os.environ.get("NISFOLDER", '/opt/SCRIPTS/findhost/data/frontix') ## files in this folder will be parsed
TEMPFOLDER = os.environ.get("TEMPFOLDER", "tmp")
TEMPDATA = os.path.join(TEMPFOLDER, "nis_summary.csv")  ## temporary output file is stored here
OUTPUTDIR = os.environ.get("CONSOLIDATOR_OUTPUT_FOLDER", "/opt/SCRIPTS/findhost/output")  ## folder where the 2 findhost files will be saved:  filtered.findhost.source.csv + findhost.source.csv
...
KEEPCOLS = [ 'SERVICEID', # VT reference
             ...
             'ACCOUNTNAME',
             'RELATED_KEYS', # find related VT's (or keys in general if another first column is used) based on ACCOUNTID + SITE_ID
             'MOBISTARREFERENCE',
             'ACCOUNTID',
             ...
             'EQUIPMENT_IPADDRESS_END', # for CPE components this is the modem end of ip address range
             'EQUIPMENTSUPPLIER',
             'SERVICESTATUS',
             'EQUIPMENT_STATUS',
             ...
             'CPE_VDSL_LP_UPDATED', ## date when the VDSL line profile was updated automatically by script
...
DUPLICATES = {
                 'CPE_WAN_SUBNET': {},  # make sure we don't have duplicate WAN IP's unless it's VDSL shared vlan on PE "emlp"
...
TRUVIEW_ACCOUNTS = [ "900154616", "900154609", "900154648", "900084334", "900136919", "900154648" ]
TRUVIEW_SITE_NAMES = {
...
def int2ip(addr):
    ...
    try:
        ...
        logger.debug("Successfully converted integer to IP ({}: {})".format(addr, ip))
    except:
        ip = addr
    ...
    if record['ACCOUNTID'] and record['SITE_ID']:
        ...
        if relationkey in RELATEDKEYS:
            if not mykey in RELATEDKEYS[relationkey]:
                RELATEDKEYS[relationkey] = COLDELIM.join((RELATEDKEYS[relationkey], mykey))
        else:
...
def alert(vt, field, column='', record=''):
    ...
    if column == 'LOOKUP_VALUE':
        if '@@' in field:
            logger.error("multiple LOOKUP_VALUE status found for {} -> {}".format(vt, field))
        if field == '' and record.get('SALESDEPARTMENT', "") != "":
            logger.error("no LOOKUPVALUE (Inactive, Completed, ..) found for {}".format(vt))
    # each VT should have a SERVICETYPE defined
    if column == 'SERVICETYPE':
        if field == '':
            logger.error("no SERVICETYPE (=IP-VPN, CI, ..) found for {}".format(vt))
    ...
    if column == 'OBS_CE_MAN_LOOPBACK' and not field == '':
        ...
        if not m:
            logger.error("OBS_CE_MAN_LOOPBACK is not correct for {} -> {}".format(vt, field))
    ...
    if column == 'SALESDEPARTMENT' and field == '' and ('completed' in record['LOOKUP_VALUE'] or 'Ongoing' in record['LOOKUP_VALUE']):
        logger.warning("SALESDEPARTMENT is not set to 312 but LOOKUP_VALUE is Completed or Ongoing for {}".format(vt))
    # multiple VRF should not exist
    if column == 'VRF' and '@@' in field:
        logger.error("same VT is used for multiple VRF -> {}: {}".format(vt, field))
    # same VT is used in different PE interfaces
    if column == 'PE_VAR_INT' and '@@' in field:
        logger.error("same VT is configured on multiple interfaces -> {}: {}".format(vt, field))
    # same VT is used on different PE devices
    if column == 'PE_VAR' and '@@' in field:
        logger.warning("same VT is configured on multiple VAR routers -> {}: {}".format(vt, field))
    ...
    if field and column in DUPLICATES and field in DUPLICATES[column] and DUPLICATES[column][field] > 0 and record['CPE_MULTIVRF'] != 'multivrf':
        logger.error("DUPLICATE entry found for column {} -> {}: {}".format(column, vt, field))
...
def customfields(field, column='', record=None):
    ...
    ## Add NPM domainid and domain names
        ...
        for tv_col in tv_columns:
           normalized_column = re.sub("[\'\"]", " ", str(record.get(tv_col, "")) )
           truview_site_name.append(normalized_column.strip())
        record['TRUVIEW_SITE_AUTOGEN'] = tv_delim.join(truview_site_name).lower().capitalize()[0:32]
    ...
    ## map some lex locations to different values
        ...
        lexmap = { '02EUR': '02MAR', '02FOR': '02DRO', '02NOH': '02MUT', '02NOR': '02MAR', '51MUE': '51MEU', '91DOS': '91OOS' }
    ...
    for line in ifile:
        ...
        if firstline:
            ...
            if not len(CH) == len(lcols):
                logger.error("line does not match the expected number of {} columns ({}) [{}]".format(len(CH), lcols, sfile))
            else:
...
def passfilter(mykey, record):
    rc = True
    if not mykey: rc = False
    #if not (record['LOOKUP_VALUE'].upper() == 'COMPLETED' or record['LOOKUP_VALUE'].upper() == 'ONGOING'): rc = False
    if not (record['LOOKUP_VALUE'].upper().find('COMPLETED') >= 0 or record['LOOKUP_VALUE'].upper().find('ONGOING') >= 0): rc = False
    return rc
...
def permute(sfile, dfile, filteredfile, keepcols):
        ...
        if firstline:
            ...
            for c in lcols:
                ...
                if vt not in cons:
                    ...
                    for k in keepcols:
                        cons[vt][k.upper()] = ''
                ch = CH[i]  ## ch = column header,  this is the current column we are looking at (matching KEEPCOLS)
                ...
                # for the same column. In that case join the fields using @@ delimiter
                    ...
                    if c is not '' and c not in cons[vt][ch]:
                        ...
                        cons[vt][ch] = "{}{}{}".format(cons[vt][ch], COLDELIM ,c)
                i = i + 1
    ...
    ffile = open(filteredfile, 'w') # filtered output file, columns stay the same but lines can be filtered
    ofile.write("{}\n".format(line0))
    ffile.write("{}\n".format(line0))
    for vt in cons:
        newline = []
        for k in keepcols:
            ...
            newline.append(customfields(cons[vt][k.upper()], k.upper(), cons[vt]))
