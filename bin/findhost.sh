...
FORMAT_BRIEF_COLS="SERVICEID SERVICETYPE CPE_HOSTNAME|CPE_HOSTNAME_GUESS|HOSTNAME CPE_LOOPBACK|OBS_CE_MAN_LOOPBACK|MOBICUSTOMERLOOPBACK|EQUIPMENT_IPADDRESS|MANAGEMENTIP|CPE_WAN_IP|LL_IPADDRESS PE_VAR PE_VAR_INT PE_VAR_INT_STATUS BANDWIDTH|CAPACITY LL_SLA"
FORMAT_BRIEF="%-6s <%s> %s [%s]|%s %s (%s) (%s) %s"
#FORMAT_BRIEF_POSTPROCESS="awk -F'|' '{printf \"%100s %s\n\",$1,$2}'"
FORMAT_BRIEF_POSTPROCESS="awk -F'|' '{printf \"%-70s %s\n\",\$1,\$2}'"

FORMAT_DETAIL_COLS="SERVICEID GSID SERVICETYPE LOOKUP_VALUE LL_SLA PARTNER ACCOUNTNAME ACCOUNTID SITE_ID SITE_SITENAME SITE_STREET SITE_ZIPCODE SITE_CITY SITE_COUNTRY TRUVIEW_SITE_AUTOGEN TRUVIEW_SITE_DESCR_AUTOGEN NPM_BW_DOWN_MB NPM_BW_UP_MB NPM_SITE_DESCR NPM_SITE_NAME DSID MACDREMARK FULLNAME RELATED_KEYS CPE_HOSTNAME|CPE_HOSTNAME_GUESS|HOSTNAME CPE_LOOPBACK|MOBICUSTOMERLOOPBACK CPE_LOOPBACK_PINGABLE EQUIPMENT_IPADDRESS EQUIPMENT_SUBNET EQUIPMENT_IPADDRESS_END MANAGEMENTIP GATEWAYIP COMPONENTID COMPONENTDESCRIPTION EQUIPMENTDESCRIPTION EQUIPMENTLABEL NETWORKELEMENT CIRCUIT_ID CIRCUIT_CAPACITY TIMESLOT TIMESLOT_B STATUS_CIRCUIT PORTLABEL_A PORTLABEL_B CES_SVLAN_ID TDI_ID PORT_UNI STATUS_TDI CAPACITY PROJECTID VDSL2TYPE DSL_SERVICE SERVICE LINEPROFILE VLANPROFILE CPE_VDSL_LP CPE_VDSL_BW_UPLOAD CPE_VDSL_BW_DOWNLOAD CPE_VDSL_LP_UPDATED CPE_VDSL_LP_MESSAGE LEX BANDWIDTH COPPERLINEID ORDERREF MAXDOWNSTREAM MAXUPSTREAM CPE_VENDOR ROUTERTYPE CPE_SW_TYPE CPE_SW_VERSION CPE_CHASSIS CPE_TRANSMISSION CPE_APN CPE_CELLULAR_IMEI CPE_CELLULAR_IMSI CPE_CELLULAR_CELLID CPE_CELLULAR_OPERATOR CPE_INTF_FUNCTION CPE_INFO_FROM DISC_DOMAINNAME DISC_COMMUNITY DISC_VENDOR DISC_HWTYPE DISC_FUNCTION DISC_SERVICE DISC_PROTOCOL PE_VAR PE_VAR_INT PE_VAR_INT_DESCR VRF COMPONENT_VPN_NAME TACACS_REALM VDSL_BW_UPSTREAM VDSL_BW_DOWNSTREAM VDSL_SVLAN_ID VDSL_SVLAN VDSL_BAS VDSL_SVLAN_PURPOSE VDSL_SVLAN_QOS_PBIT VDSL_SVLAN_BW_MBPS VDSL_SVLAN_NBR_CUSTOMERS VDSL_SVLAN_BW_PER_CUSTOMER VDSL_SVLAN_PCT_OVERBOOKING VDSL_SVLAN_NAME VDSL_SVLAN_TRUNK VDSL_LAG VDSL_CPEUSERNAME VDSL_CPEPASSWORD LL_ID LL_LINEID LL_STATUS LL_BW LL_LINETYPE LL_DSID LL_BOFID LL_OLO LL_CONTRACTENDDATE LL_CONTRACTSTARTDATE LL_CONNECTORTYPE_A LL_ZONE LL_SITEID_A LL_LOCATIONNAME_A LL_LOCATIONADDRESS_A LL_LOCATIONNAME_B LL_LOCATIONADDRESS_B LL_CONNECTORTYPE_B LL_REQUESTEDDATE LL_RFSACTUALDATE LL_CANCELDATE LL_ISHALFLINK LL_ISMULTILINK LL_SALESDEPT LABEL AUDIT_RESULT LL_IPADDRESS TACACS_CUST_REALM TACACS_CUST_TACGROUP NPM_LOGIN NPM_PSW NPM_IP NPM_DOMAIN_NAME NPM_DOMAIN_ID SVLAN_CONNECTIONID SVLAN_STATUS SVLAN_LABEL SVLAN_PRT SVLAN_CAPACITY SVLAN_TYPE SVLAN_PROJECTID_A SVLAN_PROJECTID_B SVLAN_LABEL_SHORT VSWITCH_CONNECTIONID VSWITCH_STATUS VSWITCH_LABEL VSWITCH_PRT VSWITCH_CAPACITY VSWITCH_PROJECTID_A VSWITCH_PROJECTID_B VSWITCH_LABEL_SHORT REMARKS_PROVISION REMARKS_SERVICE REMARKS_ACCOUNT REMARKS_CONTRACT PINFO_CONFIGURATION PINFO_COMMERCIAL_COS PINFO_REMARKS_ACCOUNT PINFO_REMARKS_CONTRACT PINFO_REMARKS_PROVISION PINFO_REMARKS_SERVICE PINFO_TOPOLOGY PINFO_TYPE_OF_QOS PINFO_TAGGING_OPTION PINFO_SITE_A_CVLAN PINFO_SITE_A_SVLAN PINFO_SITE_B_CVLAN PINFO_SITE_B_SVLAN"
FORMAT_DETAIL="%s: %s\n"
FORMAT_DETAIL_POSTPROCESS="cat"

FORMAT_CSV_HEADER="SERVICEID SERVICETYPE CUSTOMER CPE_HOSTNAME CPE_LOOPBACK PE_VAR PE_VAR_INT PE_VAR_INT_STATUS BANDWIDTH DSID VDSL2TYPE PARTNER"
...
function GETCONFIG {
    if [ ! -f "$USERCONFIG" ] || [ $RECREATECONFIG == 1 ]; then
        MOTD="$SCRIPT has changed! VT remarks have been added in the detail view."
        cat > ${USERCONFIG} << EOF
#
