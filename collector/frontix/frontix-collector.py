...
def read_args():
    ...
    python frontix-modinput-connector.py picklefile arg1=value1 arg2=value2
    ...
    # load the pickled file with all the parameters, delete it afterwards
...
def get_queries(query_folder, query_filter=None):
    ...
    queries = {}
    files = [ f for f in glob.glob(os.path.join(query_folder, "*.sql"), recursive=False) ]
...
def main():
    ...
    logger.debug("FRONTIX hostname:{}, user:{}".format(hostname, user))
    ...
    try:
        ...
        if postprocessor:
            ...
            ext = import_module("postprocessors.{}".format(DEFAULT_POSTPROCESSOR))
            logger.debug("using default postprocessor: {}".format(DEFAULT_POSTPROCESSOR))
    except:
        ext = None
    ...
    for q in queries:
        ...
        if m:
            if "ORDER BY" in query:
                query = query.replace("ORDER BY", "AND ROWNUM <= {} ORDER BY".format(query_limit))
            else:
        ...
        connection, columns, rows = execute_oracle_query(hostname, port, service, user, pwd, query)
        ...
        OUT = open(os.path.join(params["dump_dir"], (os.path.split(q)[-1]).replace(".sql", ".csv")), "w")
